  ...
  describe "CLI" do
    it "throws an error on invalid command" do
      expect { @machinery.run_command(
        "cd; machinery/machinery/bin/machinery invalid_command",
        :as => "vagrant",
        :stdout => :capture
    ...
    it "processes help option" do
      output = @machinery.run_command(
        "cd; machinery/machinery/bin/machinery -h",
        :as => "vagrant",
        :stdout => :capture
      )
    ...
    it "processes help option for subcommands" do
      output = @machinery.run_command(
        "cd; machinery/machinery/bin/machinery inspect --help",
        :as => "vagrant",
        :stdout => :capture
      )
    ...
    it "does not offer --no-help or unneccessary negatable options" do
      global_output = @machinery.run_command(
        "cd; machinery/machinery/bin/machinery --help",
        :as => "vagrant",
        :stdout => :capture
      )
    ...
    describe "inspect" do
      it "fails inspect for non existing scope" do
        expect { @machinery.run_command(
          "cd; sudo machinery/machinery/bin/machinery inspect localhost --scope=foobar --name=test",
          :as => "vagrant",
          :stdout => :capture
    ...
    describe "build" do
      it "fails without an output path" do
        expect { @machinery.run_command(
          "cd; machinery/machinery/bin/machinery build test",
          :as => "vagrant",
          :stdout => :capture
      ...
      it "fails without a name" do
        expect { @machinery.run_command(
          "cd; machinery/machinery/bin/machinery build --image-dir=/tmp/",
          :as => "vagrant",
          :stdout => :capture
