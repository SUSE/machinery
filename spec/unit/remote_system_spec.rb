  ...
  context "connecting to a remote system" do
    ...
    describe "#run_command" do
      it "executes commands via ssh" do
        expect(Cheetah).to receive(:run).with("ssh", "root@remotehost", "LC_ALL=en_US.utf8", "ls", "/tmp", {})

        remote_system.run_command("ls", "/tmp")
      end

      it "executes piped commands via ssh" do
        expect(Cheetah).to receive(:run).with("ssh", "root@remotehost", "LC_ALL=en_US.utf8", "ls", "/tmp", "|", "grep", "foo", "|", "wc", "-l", {})
      ...
      it "uses sudo when necessary" do
        expect(Cheetah).to receive(:run).with(
          "ssh", "machinery@remotehost", "sudo", "-n", "LC_ALL=en_US.utf8", "ls", "/tmp", privileged: true
        )
